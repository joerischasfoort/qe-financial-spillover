...
def spillover_model_ELA(portfolios, currencies, environment, exogeneous_agents, funds,  seed, obj_label):
    """
    ...
    ######################################################################
    hex_home = '/home/jriedler/qe-financial-spillover/Experiments/Elasticity/Objects_ELA/'
    ...
    for day in range(environment.par.global_parameters['start_day'], environment.par.global_parameters['end_day']):
        ...
        while intraday_over == False:
            ...
            if tau > 10000:
                dir = local_dir
                file_name = hex_home + '!noCONV_objects_day_' + str(day) + "_seed_" + str(seed) + "_" + obj_label +  '.pkl'
        ...
        for fund in funds:
            fund.var.ewma_returns, fund.var.covariance_matrix, fund.var.hypothetical_returns = covariance_estimate(fund,  environment, previous_cons_returns[fund], inflation_shock)
        ...
        # saving objects
        if day>=4000:
            file_name = hex_home + '/objects_day_' + str(day) + "_seed_" + str(seed) + "_" + obj_label + '.pkl'
            save_objects = open(file_name, 'wb')
