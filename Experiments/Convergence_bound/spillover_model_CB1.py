...
def spillover_model_CB1(portfolios, currencies, environment, exogeneous_agents, funds,  seed, obj_label):
    ...
    ######################################################################
    hex_home = '/home/jriedler/qe-financial-spillover/Experiments/Convergence_bound/Objects_CB1/'
    ...
    for day in range(environment.par.global_parameters['start_day'], environment.par.global_parameters['end_day']):
        ...
        for row in environment.var.fx_rates.index:
            for col in environment.var.fx_rates.columns:
                environment.var.ewma_fx_rates.loc[row, col] = compute_ewma(environment.var.fx_rates.loc[row, col], environment.var.ewma_fx_rates.loc[row, col],0.0)
