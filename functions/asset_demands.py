...
def asset_demand_oc(fund, portfolios, currencies):
    """
    ...
    for a in portfolios:
        ...
        a_demand[a] = ((fund.var.weights[a] * fund.var.redeemable_shares) / (a.var.price)) - out * fund.var_previous.assets[a]

    c_demand = {}
    for c in currencies:  #
        c_demand[c] = (fund.var.weights[c] * fund.var.redeemable_shares) - fund.var.currency_inventory[c]
