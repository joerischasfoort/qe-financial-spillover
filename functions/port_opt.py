...
def portfolio_optimization_oc(fund, day):
    """
    Find the optimal portfolio weights for a fund.
    :param fund: Fund object
    :param day: integer day at which the simulation takes place
    ...
    # compute the risk aversion matrix TODO change this
    risk_aversion_mat = pd.DataFrame((np.zeros((len(covariance_assets), len(covariance_assets))) + fund.par.risk_aversion['domestic_assets']), index=covariance_assets.columns,
    ...
    for i, a in enumerate(covariance_assets.columns.values):
        expected_return_assets[i] = fund.exp.returns[a][day]
