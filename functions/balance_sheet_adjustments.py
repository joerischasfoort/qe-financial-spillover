...
def fund_asset_adjustments_oc(fund, portfolios, excess_demand, pi, nu, day):
    new_position = {}
    for a in portfolios:
        ...
        # compute new balance sheet position of funds
        if fund.var.asset_demand[a] > 0 and excess_demand[a] > 0:
            new_position[a] = out * fund.var.assets[a][day - 1] + fund.var.asset_demand[a] * pi[a]

        elif fund.var.asset_demand[a] < 0 and excess_demand[a] < 0:
            new_position[a] = out * fund.var.assets[a][day - 1] + fund.var.asset_demand[a] * nu[a]

        else:
            new_position[a] = out * fund.var.assets[a][day - 1] + fund.var.asset_demand[a]
...
def ex_asset_adjustments_oc(ex, portfolios, excess_demand, pi, nu, exogeneous_agents, day):
    new_position = {}
    for a in portfolios:
        out = a.par.maturity * (1 - a.var.default_rate[day])
        if ex == "central_bank_domestic":
            # compute new balance sheet position of exogenous agent
            if exogeneous_agents[ex].var.asset_demand[a] > 0 and excess_demand[a] > 0:
                new_position[a] = out * exogeneous_agents[ex].var.assets[a][day - 1] + \
                                  exogeneous_agents[ex].var.asset_demand[a] * pi[a]

            elif exogeneous_agents[ex].var.asset_demand[a] < 0 and excess_demand[a] < 0:
                new_position[a] = out * exogeneous_agents[ex].var.assets[a][day - 1] + \
                                  exogeneous_agents[ex].var.asset_demand[a] * nu[a]

            else:
                new_position[a] = out * exogeneous_agents[ex].var.assets[a][day - 1] + \
                                  exogeneous_agents[ex].var.asset_demand[a]
...
def cash_demand_correction_oc(fund, currencies):
    new_cash_demand = {c: 0 for c in currencies}
    for c, cu in zip(currencies, reversed(currencies)):
        if np.sign(fund.var.currency_demand[c]) != np.sign(fund.var.currency_demand[cu]):
            new_cash_demand[c] = np.sign(fund.var.currency_demand[c]) * min(abs(fund.var.currency_demand[c]), abs(fund.var.currency_demand[cu]))
...
def cash_excess_demand_and_correction_factors_oc(funds, currencies, exogeneous_agents):
    # compute correcting factors for portfolios of assets
    piC = {}
    nuC = {}
    excess_demandC = {}
...
def fund_cash_inventory_adjustment_oc(fund, portfolios, currencies, day):
    net_cash_flows = {c : 0 for c in currencies}
    new_cash_inventory = {}
    ...
    for c in currencies:
        for a in portfolios:
            out = a.par.maturity * (1 - a.var.default_rate[day])
            net_cash_flows[c] = net_cash_flows[c] + (out * (fund.var.assets[a][day - 1]) - fund.var.assets[a][day]) * a.var.price[day] #TODO are the days correct?
