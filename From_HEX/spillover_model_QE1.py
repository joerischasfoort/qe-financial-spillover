...
def spillover_model_QE1(portfolios, currencies, environment, exogeneous_agents, funds,  seed, obj_label):
    ...
    ######################################################################
    hex_home = '/home/jriedler/qe-financial-spillover/Experiments/QE/Objects_QE1/'
	...
	# these two variables are needed for the pricing algorithm
        ...
        for row in environment.var.fx_rates.index:
            for col in environment.var.fx_rates.columns:
                environment.var.ewma_fx_rates.loc[row, col] = compute_ewma(environment.var.fx_rates.loc[row, col], environment.var.ewma_fx_rates.loc[row, col],1.0)
